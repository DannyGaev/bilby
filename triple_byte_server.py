...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Look for the format sent by dns python
        if "00000c0001" in hex_message:
            is_baseline, val = find_payload(hex_message)
            # If the current payload determines the baseline, set it accordingly
            if is_baseline:
                new_baseline = val
                print(f"[!] NEW BASELINE: {new_baseline}")
                create_response(data, address)
            else:
                # Determine the current stream id
                stream_id = hex_message[0:4]
                if stream_id != old_stream_id:
                    # Check for hardcoded termination value
                    if ".".join(val) == "104.6.7.2":
